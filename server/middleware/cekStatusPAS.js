...
// Middleware: validasi status PAS, inject req.pasStatus/pasLocked/pasNotOpen, blokir write jika tidak aktif
const cekStatusPAS = async (req, res, next) => {
    try {
        ...
        if (status_pas === 'selesai') {
            if (isWriteOperation) {
                return res.status(403).json({ success: false, message: 'Input ekstrakurikuler sudah dikunci. PAS telah selesai.', code: 'PERIOD_LOCKED' });
            }
            req.pasLocked = true;
        ...
        if (isWriteOperation) {
            return res.status(403).json({ success: false, message: 'Input ekstrakurikuler hanya tersedia saat PAS aktif. Silakan hubungi admin untuk membuka periode PAS.', code: 'PERIOD_NOT_OPEN' });
        }
        ...
        res.status(500).json({ success: false, message: 'Gagal memeriksa status PAS', code: 'SYSTEM_ERROR' });
    }
};
