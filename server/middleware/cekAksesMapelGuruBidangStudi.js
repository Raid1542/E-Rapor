...
// Middleware: cek penugasan guru bidang studi ke mapel dan inject req.penugasanMapel
const cekAksesMapelGuruBidangStudi = async (req, res, next) => {
    try {
        ...
        // Ambil mapelId dari berbagai sumber (params/query/body/DELETE)
        ...
        // Cek penugasan guru ke mapel di semester aktif
        ...
        if (!rows || rows.length === 0) {
            return res.status(403).json({
                success: false,
                message: 'Anda tidak mengajar mapel ini di semester aktif.',
                code: 'NO_ACCESS_TO_MAPEL',
            });
        }

        // Inject informasi penugasan ke request
        req.penugasanMapel = {
