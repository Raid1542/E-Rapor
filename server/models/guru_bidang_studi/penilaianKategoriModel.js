...
const getKategoriByMapel = async (mapelId, semesterId, kelasId, jenisPenilaian = null) => {
    let query = 'SELECT id_config AS id, min_nilai, max_nilai, deskripsi, urutan, kelas_id, jenis_penilaian FROM konfigurasi_nilai_rapor WHERE mapel_id = ? AND tahun_ajaran_id = ? AND kelas_id = ?';
    const params = [mapelId, semesterId, kelasId];
    if (jenisPenilaian && ['PTS', 'PAS'].includes(jenisPenilaian)) { query += ' AND jenis_penilaian = ?'; params.push(jenisPenilaian); }
...
const getLastUrutan = async (mapelId, semesterId, kelasId) => {
    const [rows] = await db.execute(
        'SELECT IFNULL(MAX(urutan), 0) as max_urutan FROM konfigurasi_nilai_rapor WHERE mapel_id = ? AND tahun_ajaran_id = ? AND kelas_id = ?',
...
const cekRangeOverlap = async (mapelId, semesterId, minNilai, maxNilai, kelasId, excludeId = null, jenisPenilaian = null) => {
    let query = 'SELECT id_config, min_nilai, max_nilai, deskripsi FROM konfigurasi_nilai_rapor WHERE mapel_id = ? AND tahun_ajaran_id = ? AND kelas_id = ? AND (? <= max_nilai AND ? >= min_nilai)';
...
const cekCoverage0to100 = async (mapelId, semesterId, kelasId, jenisPenilaian = null) => {
    let query = 'SELECT min_nilai, max_nilai FROM konfigurasi_nilai_rapor WHERE mapel_id = ? AND tahun_ajaran_id = ? AND kelas_id = ?';
    const params = [mapelId, semesterId, kelasId];
    if (jenisPenilaian && ['PTS', 'PAS'].includes(jenisPenilaian)) { query += ' AND jenis_penilaian = ?'; params.push(jenisPenilaian); }
    query += ' ORDER BY min_nilai ASC';
    const [kategoriRows] = await db.execute(query, params);
    const gaps = [];
    if (kategoriRows.length === 0) return { covered: false, gaps: [{ aspek: 'Akademik', gap: '0-100' }] };
    if (kategoriRows[0].min_nilai > 0) gaps.push({ aspek: 'Akademik', gap: `0-${kategoriRows[0].min_nilai - 1}` });
    for (let i = 0; i < kategoriRows.length - 1; i++) {
        const currentMax = kategoriRows[i].max_nilai;
        const nextMin = kategoriRows[i + 1].min_nilai;
        if (nextMin > currentMax + 1) gaps.push({ aspek: 'Akademik', gap: `${currentMax + 1}-${nextMin - 1}` });
    }
    const lastMax = kategoriRows[kategoriRows.length - 1].max_nilai;
    if (lastMax < 100) gaps.push({ aspek: 'Akademik', gap: `${lastMax + 1}-100` });
...
const updateKategori = async (id, data) => {
    ...
    const [result] = await db.execute(
        'UPDATE konfigurasi_nilai_rapor SET min_nilai = ?, max_nilai = ?, deskripsi = ?, urutan = ? WHERE id_config = ?',
...
const deleteKategori = async (id) => {
    const [result] = await db.execute('DELETE FROM konfigurasi_nilai_rapor WHERE id_config = ?', [id]);
    return result.affectedRows;
};
...
const isUnchanged = (oldData, newData) => {
    return oldData.min_nilai === newData.min_nilai && oldData.max_nilai === newData.max_nilai && oldData.deskripsi.trim() === newData.deskripsi.trim();
};
