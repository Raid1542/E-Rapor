...
/**
 * Cek Range Overlapping di konfigurasi_nilai_rapor
 */
 ...
 */
exports.cekGuruMengajarMapel = async (userId, mapelId, tahunAjaranIndukId) => {
...
exports.getKomponenPenilaianList = async () => {
    const [komponen] = await db.execute(`
        SELECT id_komponen, nama_komponen, urutan
        FROM komponen_penilaian
        ORDER BY urutan ASC
...
exports.cekKomponenValid = async (komponenIds) => {
    ...
    `);
    const validIds = new Set(komponenRows.map(k => k.id_komponen));
    return komponenIds.every(id => validIds.has(id));
};