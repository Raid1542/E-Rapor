...
// Ambil semua aspek kokurikuler (Mutaba'ah, BPI, Literasi, Proyek)
exports.getAspekKokurikuler = async () => {
    const [aspek] = await db.execute('SELECT id_aspek_kokurikuler, kode, nama FROM aspek_kokurikuler ORDER BY urutan ASC');
    return aspek;
};
...
// Cek overlap range di konfigurasi nilai rapor (per kelas + jenis)
exports.cekOverlapAkademik = async (mapelId, tahunAjaranId, kelasId, minNilai, maxNilai, jenis = 'PTS', excludeId = null) => {
    ...
    if (mapelId === null) query += ' AND mapel_id IS NULL';
    else { query += ' AND mapel_id = ?'; params.push(mapelId); }
    if (excludeId) { query += ' AND id_config != ?'; params.push(excludeId); }
    const [overlaps] = await db.execute(query, params);
    return overlaps;
};

// Cek overlap range di kategori grade kokurikuler (per kelas + jenis)
exports.cekOverlapKokurikuler = async (idAspek, tahunAjaranId, semester, kelasId, minNilai, maxNilai, jenis = 'PTS', excludeId = null) => {
    ...
    if (excludeId) { query += ' AND id_kategori_grade_kokurikuler != ?'; params.push(excludeId); }
    const [overlaps] = await db.execute(query, params);
    return overlaps;
};

// Cek duplikasi grade kokurikuler (per kelas + jenis)
...
// Cek coverage 0-100 di konfigurasi nilai rapor (per kelas + jenis)
exports.cekCoverage0to100 = async (mapelId, tahunAjaranId, kelasId, jenis = 'PTS') => {
    ...
    if (kategoriRows[0].min_nilai > 0) return { covered: false, gap: `0-${kategoriRows[0].min_nilai - 1}` };
...
// Cek apakah kategori sedang dipakai di nilai rapor (per kelas + jenis)
exports.cekKategoriDipakai = async (mapelId, tahunAjaranId, kelasId, minNilai, maxNilai, jenis = 'PTS') => {
    if (mapelId === null) return { total: 0 };
    const [rows] = await db.execute(
...
exports.cekKomponenValid = async (komponenIds) => {
    if (!Array.isArray(komponenIds) || komponenIds.length === 0) return false;
    const placeholders = komponenIds.map(() => '?').join(',');
    const [rows] = await db.execute(`SELECT id_komponen FROM komponen_penilaian WHERE id_komponen IN (${placeholders})`, komponenIds);
...
// Ambil kategori akademik per mapel + per kelas + per jenis
...
// Hapus kategori akademik dengan filter kelas_id
...
// Ambil kategori kokurikuler per kelas + per jenis
...
// Ambil kategori kokurikuler by ID + kelas (untuk keamanan)
...
// Hapus kategori kokurikuler dengan filter kelas_id
...
// Ambil bobot per mapel + per kelas + per jenis
...
// Simpan bobot dengan kelas_id + jenis_penilaian (transaction)
exports.saveBobot = async (mapelId, kelasId, tahunAjaranId, bobotList, jenis = 'PTS') => {
    const connection = await db.getConnection();
    try {
        await connection.beginTransaction();
        await connection.execute('DELETE FROM konfigurasi_mapel_komponen WHERE mapel_id = ? AND kelas_id = ? AND jenis_penilaian = ?', [mapelId, kelasId, jenis]);
...
// Cek coverage 0-100 untuk setiap aspek kokurikuler (per kelas + jenis)
...
// Cek kategori kokurikuler dipakai (per kelas)
exports.cekKategoriKokurikulerDipakai = async (idKategori, kelasId) => {
    ...
    if (kategoriRows.length === 0) return { total: 0, exists: false };
    ...
    if (!kolom) return { total: 0, exists: true, error: `Aspek ID ${id_aspek_kokurikuler} tidak valid` };
    const [rows] = await db.execute('SELECT COUNT(*) as total FROM nilai_kokurikuler WHERE kelas_id = ? AND ${kolom} BETWEEN ? AND ?', [kelasId, rentang_min, rentang_max]);
...
// Ambil kategori deskripsi rata-rata (hanya PTS)
...
exports.cekOverlapDeskripsiRataRata = async (tahunAjaranId, semester, kelasId, minNilai, maxNilai, excludeId = null) => {
    ...
    if (excludeId) { query += ' AND id_kategori != ?'; params.push(excludeId); }
    const [overlaps] = await db.execute(query, params);
    return overlaps;
};
