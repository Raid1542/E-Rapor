 ...
 * Fungsi: Model judul proyek kokurikuler per kelas
...
exports.getJudulProyekByKelas = async (kelasId, tahunAjaranId) => {
    const [rows] = await db.execute(
        'SELECT id_judul_proyek, id_tahun_ajaran, kelas_id, judul FROM judul_proyek_per_tahun_ajaran WHERE kelas_id = ? AND id_tahun_ajaran = ? LIMIT 1',
...
exports.saveJudulProyek = async (kelasId, tahunAjaranId, judul) => {
    ...
    if (existing.length > 0) {
        await db.execute(
            'UPDATE judul_proyek_per_tahun_ajaran SET judul = ?, updated_at = NOW() WHERE kelas_id = ? AND id_tahun_ajaran = ?',
            [judul, kelasId, tahunAjaranId]
        );
        return { id: existing[0].id_judul_proyek, action: 'updated' };
    } else {
        const [result] = await db.execute(
            'INSERT INTO judul_proyek_per_tahun_ajaran (id_tahun_ajaran, kelas_id, judul, created_at, updated_at) VALUES (?, ?, ?, NOW(), NOW())',
...
exports.deleteJudulProyek = async (kelasId, tahunAjaranId) => {
    const [result] = await db.execute(
        'DELETE FROM judul_proyek_per_tahun_ajaran WHERE kelas_id = ? AND id_tahun_ajaran = ?',
        [kelasId, tahunAjaranId]
    );
