 ...
 * Fungsi: Controller catatan wali kelas per siswa (sanitasi XSS, validasi minimal, pre-fill data)
 * Pembuat: Raid Aqil Athallah - NIM: 3312401022
 * Tanggal: 10 Juli 2026
 * Update: 15 Juli 2026 - Hapus validasi maksimal karakter, pastikan pre-fill data berdasarkan jenis & semester aktif
 */
...
// Hitung kesamaan string (Levenshtein Distance) untuk toleransi typo
...
exports.updateCatatanWaliKelas = async (req, res) => {
    try {
        ...
        // ✅ DIHAPUS: Validasi maksimal 500 karakter sesuai permintaan
...
exports.downloadTemplateCatatanWali = async (req, res) => {
    try {
        ...
        // ✅ PRE-FILL: Ambil data catatan yang sudah ada di database untuk jenis & semester ini
        const [catatanRows] = await db.execute(
            `SELECT c.siswa_id, c.catatan_wali_kelas, c.naik_tingkat
             FROM catatan_wali_kelas c
             JOIN siswa_kelas sk ON c.siswa_id = sk.siswa_id
             WHERE sk.kelas_id = ? AND sk.id_tahun_ajaran_induk = ? AND c.semester = ? AND c.jenis_penilaian = ?`,
            [kelasId, indukId, semesterName, jenisPenilaian]
        );

        const catatanMap = {};
        catatanRows.forEach(c => {
            catatanMap[c.siswa_id] = { 
                catatan: c.catatan_wali_kelas || '', 
                naik_tingkat: c.naik_tingkat || '' 
            };
        });
        ...
        siswaRows.forEach((siswa, index) => {
            ...
            // ✅ Ambil data existing untuk siswa ini
            const existing = catatanMap[siswa.id_siswa] || {};
            ...
            // ✅ PRE-FILL: Isi kolom catatan dengan data dari database
            const catatanCell = dataRow.getCell(5);
            catatanCell.value = existing.catatan;
            ...
            catatanCell.border = { top: { style: 'thin', color: { argb: 'FFCCCCCC' } }, left: { style: 'thin', color: { argb: 'FFCCCCCC' } }, bottom: { style: 'thin', color: { argb: 'FFCCCCCC' } }, right: { style: 'thin', color: { argb: 'FFCCCCCC' } } };
            catatanCell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: isEvenRow ? 'FFFFF5E6' : 'FFFFFFFF' } };

            // Kolom naik tingkat
            const naikTingkatCell = dataRow.getCell(6);
            if (jenisPenilaian === 'PAS' && semesterName === 'Genap') {
                naikTingkatCell.value = existing.naik_tingkat;
            ...
            naikTingkatCell.border = { top: { style: 'thin', color: { argb: 'FFCCCCCC' } }, left: { style: 'thin', color: { argb: 'FFCCCCCC' } }, bottom: { style: 'thin', color: { argb: 'FFCCCCCC' } }, right: { style: 'thin', color: { argb: 'FFCCCCCC' } } };
            naikTingkatCell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: isEvenRow ? 'FFFFF5E6' : 'FFFFFFFF' } };
        });
...
// ═════════════════════════════════════════════════════════════════════════════
// 4. IMPORT CATATAN WALI DARI EXCEL (DENGAN SKIP KOSONG)
// ═════════════════════════════════════════════════════════════════════════════
...
exports.importCatatanWaliExcel = async (req, res) => {
    ...
    try {
        ...
        const [taRows] = await db.execute("SELECT id_tahun_ajaran, id_tahun_ajaran_induk, status_pts, status_pas FROM tahun_ajaran WHERE status = 'aktif' LIMIT 1");
        if (taRows.length === 0) return res.status(400).json({ success: false, message: 'Tahun ajaran aktif belum diatur' });
        ...
        if ((jenisPenilaian === 'PTS' && status_pts !== 'aktif') || (jenisPenilaian === 'PAS' && status_pas !== 'aktif')) {
            return res.status(403).json({ success: false, message: `Periode ${jenisPenilaian} sudah dikunci atau belum dibuka. Import tidak dapat dilakukan.` });
        }
        ...
        if (data.length < 2) return res.status(400).json({ success: false, message: 'File Excel tidak valid. Minimal harus ada header dan 1 baris data.' });
        ...
        if (!adaDataSiswa) {
            return res.status(400).json({
                success: false, message: 'File Excel tidak valid - tidak ada data siswa.\n\nSolusi:\n1. Download ulang template\n2. Pastikan kolom NIS dan Nama Siswa terisi\n3. Upload kembali file yang sudah diisi',
                data: { total_baris: data.length - dataStartIndex, berhasil: 0, gagal: 0, dilewati: data.length - dataStartIndex, errors: null, warnings: [{ row: 0, message: 'File Excel tidak berisi data siswa.' }], periode: `${jenisPenilaian} ${semesterName}` }
            });
        }
        ...
        if (jenisPenilaian !== 'PAS' || semesterName !== 'Genap') {
            warnings.push({ row: 0, message: 'Kolom "Naik Tingkat" diabaikan karena hanya berlaku untuk PAS Semester Genap.' });
        }
        ...
        for (let i = dataStartIndex; i < data.length; i++) {
            ...
            if (idxNISN >= 0) {
                ...
                if (nisnExcel && nisnDiproses.has(nisnExcel)) {
                    warnings.push({ row: i + 1, message: `Baris ${i + 1}: NISN "${nisnExcel}" DUPLIKAT. Hanya data pertama yang diproses.` });
                    skippedCount++;
                    continue;
                }
                if (nisnExcel) nisnDiproses.add(nisnExcel);
            }
            ...
            // ✅ SKIP: Jika kosong atau '-', lewati baris ini (anggap tidak ada perubahan)
            if (!catatan || catatan === '-') {
                warnings.push({ row: i + 1, message: `Baris ${i + 1}: Catatan kosong untuk "${siswa.nama_lengkap}". Data dilewati (tidak diubah).` });
                skippedCount++;
                continue;
            }
            ...
            await connection.execute(
                `INSERT INTO catatan_wali_kelas 
                 ...
                 naik_tingkat = VALUES(naik_tingkat),
                 updated_at = NOW()`,
                [siswa.id_siswa, kelasId, semesterId, semesterName, jenisPenilaian, sanitizedCatatan, naikTingkatValue]
            );
        ...
        if (errors.length > 0) {
            ...
            message = successCount > 0 
                ? `Import sebagian berhasil: ${successCount} catatan disimpan, tetapi ada ${errors.length} error.` 
            ...
            message = 'Tidak ada data yang berhasil diimport (semua baris dilewati).';
        }
        ...
        res.json({
            ...
            data: {
                ...
                baris_dengan_data_siswa: skippedCount + successCount + errors.length,
            },
        });
    } catch (err) {
        await connection.rollback();
        console.error('Error importCatatanWaliExcel:', err);
        res.status(500).json({ success: false, message: 'Gagal mengimport catatan wali kelas: ' + err.message });
    } finally {
        connection.release();
    }
};