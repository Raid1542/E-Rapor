...
const kokurikulerModel = {
  ...
  async getSiswaByKelas(kelasId, tahunAjaranId) {
    const [rows] = await db.execute(
      'SELECT s.id_siswa, s.nama_lengkap, s.nis, s.nisn FROM siswa s INNER JOIN siswa_kelas sk ON s.id_siswa = sk.siswa_id WHERE sk.kelas_id = ? AND sk.id_tahun_ajaran_induk = ? ORDER BY s.nama_lengkap',
  ...
  async getNilaiByKelas(kelasId, tahunAjaranId, semester, jenisPenilaian) {
    const [rows] = await db.execute(
      'SELECT id_nilai_kokurikuler, id_siswa, id_aspek_kokurikuler, nilai, grade, deskripsi, id_judul_proyek, jenis_penilaian FROM nilai_kokurikuler WHERE id_kelas = ? AND id_tahun_ajaran = ? AND semester = ? AND jenis_penilaian = ?',
  ...
  async getNilaiBySiswa(siswaId, kelasId, tahunAjaranId, semester, jenisPenilaian) {
    const [rows] = await db.execute(
      'SELECT id_nilai_kokurikuler, id_aspek_kokurikuler, nilai, grade, deskripsi, id_judul_proyek, jenis_penilaian FROM nilai_kokurikuler WHERE id_siswa = ? AND id_kelas = ? AND id_tahun_ajaran = ? AND semester = ? AND jenis_penilaian = ?',
  ...
  async checkExistingNilai(siswaId, aspekId, kelasId, tahunAjaranId, semester, jenisPenilaian) {
    const [rows] = await db.execute(
      'SELECT id_nilai_kokurikuler FROM nilai_kokurikuler WHERE id_siswa = ? AND id_aspek_kokurikuler = ? AND id_kelas = ? AND id_tahun_ajaran = ? AND semester = ? AND jenis_penilaian = ?',
  ...
  async updateNilai(idNilaiKokurikuler, nilai, grade, deskripsi, idJudulProyek = null) {
    await db.execute(
      'UPDATE nilai_kokurikuler SET nilai = ?, grade = ?, deskripsi = ?, id_judul_proyek = ?, updated_at = NOW() WHERE id_nilai_kokurikuler = ?',
  ...
  async insertNilai(siswaId, aspekId, kelasId, tahunAjaranId, semester, jenisPenilaian, nilai, grade, deskripsi, idJudulProyek = null) {
    const [result] = await db.execute(
      'INSERT INTO nilai_kokurikuler (id_siswa, id_aspek_kokurikuler, id_kelas, id_tahun_ajaran, semester, jenis_penilaian, nilai, grade, deskripsi, id_judul_proyek) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
  ...
  async getKonfigurasiGradeByAspek(aspekId, kelasId, tahunAjaranId, semester, jenisPenilaian) {
    const [rows] = await db.execute(
      'SELECT id_kategori_grade_kokurikuler, id_aspek_kokurikuler, rentang_min, rentang_max, grade, deskripsi FROM kategori_grade_kokurikuler WHERE id_aspek_kokurikuler = ? AND kelas_id = ? AND tahun_ajaran_id = ? AND semester = ? AND jenis_penilaian = ? ORDER BY rentang_min DESC',
  ...
  hitungGrade(nilai, konfigurasiGrade) {
    ...
    const gradeConfig = konfigurasiGrade.find(g => nilai >= parseFloat(g.rentang_min) && nilai <= parseFloat(g.rentang_max));
