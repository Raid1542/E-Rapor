...
const tahunAjaranModel = {
  ...
  async gantiSemesterAktif(id_induk, semester_baru, connection = db) {
    ...
    try {
      await conn.beginTransaction();
      await conn.execute('UPDATE tahun_ajaran SET status = \'nonaktif\' WHERE id_tahun_ajaran_induk = ?', [id_induk]);
      await conn.execute('UPDATE tahun_ajaran SET status = \'aktif\' WHERE id_tahun_ajaran_induk = ? AND semester = ?', [id_induk, semester_baru]);
      await conn.commit();
      return true;
    } catch (err) {
      await conn.rollback();
