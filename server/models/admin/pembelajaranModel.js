...
const pembelajaranModel = {
  ...
  /** Ambil pembelajaran by kelas (opsional filter tahun ajaran) */
  async getByKelasId(kelasId, tahunAjaranId = null) {
    ...
    if (tahunAjaranId) { sql += ' AND p.tahun_ajaran_id = ?'; params.push(tahunAjaranId); }
  ...
  async deleteById(id, connection = db) {
    const [result] = await connection.execute('DELETE FROM pembelajaran WHERE id = ?', [id]);
    return result.affectedRows > 0;
  },
  ...
  async getGuruPengampuMapelDiKelas(mapel_id, kelas_id, tahun_ajaran_id, excludeUserId = null) {
    ...
    if (excludeUserId != null) { sql += ' AND p.user_id != ?'; params.push(excludeUserId); }
    const [rows] = await db.execute(sql, params);
    return rows[0] || null;
  },
