...
const pembelajaranModel = {
  ...
  async create(data, connection = db) {
    ...
    const [result] = await connection.execute(
      `INSERT INTO pembelajaran 
        (tahun_ajaran_id, kelas_id, mapel_id, user_id, created_at, updated_at)
        VALUES (?, ?, ?, ?, NOW(), NOW())`,
  ...
  async update(id, data, connection = db) {
    ...
    const [result] = await connection.execute(
      `UPDATE pembelajaran 
        SET kelas_id = ?, mapel_id = ?, user_id = ?, updated_at = NOW()
        WHERE id = ?`,
  ...
  async isDuplicate(user_id, kelas_id, mapel_id, tahun_ajaran_id, excludeId = null) {
    let sql = `
      ...
      WHERE user_id = ? AND kelas_id = ? AND mapel_id = ? AND tahun_ajaran_id = ?
    `;
  ...
  async getGuruPengampuMapelDiKelas(mapel_id, kelas_id, tahun_ajaran_id, excludeUserId = null) {
    let sql = `
      ...
      FROM pembelajaran p
      JOIN user u ON p.user_id = u.id_user
      WHERE p.mapel_id = ? AND p.kelas_id = ? AND p.tahun_ajaran_id = ?
    `;
  ...
  async isWaliKelas(userId, kelasId, tahunAjaranId) {
    const [rows] = await db.execute(
      ...
      WHERE user_id = ? AND kelas_id = ? AND tahun_ajaran_id = ?`,
