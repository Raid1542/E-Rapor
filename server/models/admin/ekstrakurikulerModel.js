...
const ekstrakurikulerModel = {
  ...
  async deleteById(id) {
    ...
    if (peserta.length > 0) throw new Error('Ekstrakurikuler masih memiliki anggota');
  ...
  async getAllPembinaAktif() {
    const [rows] = await db.execute(
      `SELECT id_pembina_ekstrakurikuler AS id, nama_lengkap AS nama 
        FROM pembina_ekstrakurikuler WHERE status = 'aktif' ORDER BY nama_lengkap ASC`
    );
    return rows;
  },
  ...
  async getSiswaInKelas(kelasId, tahunAjaranId) {
    const [rows] = await db.execute(
      `SELECT s.id_siswa, s.nama_lengkap AS nama, s.nis, s.nisn
        ...
        WHERE sk.kelas_id = ? AND sk.tahun_ajaran_id = ? ORDER BY s.nama_lengkap`,
