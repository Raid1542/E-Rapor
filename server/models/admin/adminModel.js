...
const adminModel = {
    ...
    async createAdmin(userData, connection = db) {
        const {
            ...
            niy = '', nuptk = '', tempat_lahir = '', tanggal_lahir = null,
            jenis_kelamin = 'Laki-laki', alamat = '', no_telepon = '',
        } = userData;
        ...
        await connection.execute('INSERT INTO user_role (id_user, role) VALUES (?, "admin")', [id_user]);
    ...
    async bulkInsertMapelWajib(kelasId, mapelIds, guruKelasId, tahunAjaranId, connection) {
        ...
        for (const mapelId of mapelIds) {
            ...
            if (cek.length === 0) {
                await connection.execute(
                    `INSERT INTO pembelajaran (kelas_id, mapel_id, user_id, tahun_ajaran_id, created_at, updated_at) VALUES (?, ?, ?, ?, NOW(), NOW())`,
