 ...
 * UPDATE: Perbaiki logika tahun_ajaran_id - kelas per tahun ajaran induk
...
const kelasModel = {
  /** Ambil semua kelas (opsional filter tahun ajaran induk) */
  ...
  async getById(id) {
    const [rows] = await db.execute(`
      SELECT k.*, COALESCE(gk.user_id, NULL) AS wali_kelas_id
      FROM kelas k
  ...
  async getByIdWithDetails(id, tahunAjaranIdInduk) {
    // ✅ PERBAIKAN: tahunAjaranIdInduk sekarang adalah id_tahun_ajaran_induk
    const [rows] = await db.execute(`
      SELECT 
  ...
  async create(data) {
    ...
    if (!nama_kelas || !fase || !tahun_ajaran_id) throw new Error('Nama kelas, fase, dan tahun_ajaran_id wajib diisi');
    if (!VALID_FASE.includes(fase)) throw new Error(`Fase tidak valid. Pilih dari: ${VALID_FASE.join(', ')}`);

    const [existing] = await db.execute(
      'SELECT id_kelas FROM kelas WHERE LOWER(nama_kelas) = LOWER(?) AND tahun_ajaran_id = ?',
      [nama_kelas.trim(), tahun_ajaran_id]
    );
    if (existing.length > 0) throw new Error(`Kelas "${nama_kelas}" sudah ada di tahun ajaran ini`);
  ...
  async update(id, data) {
    ...
    if (existing.length > 0) throw new Error(`Nama kelas "${nama_kelas}" sudah digunakan di tahun ajaran ini`);
  ...
  /** Ambil semua kelas per tahun ajaran induk */
  async getByTahunAjaran(tahun_ajaran_id_induk) {
    if (!tahun_ajaran_id_induk) throw new Error('tahun_ajaran_id_induk wajib diisi');
    const [rows] = await db.execute('SELECT * FROM kelas WHERE tahun_ajaran_id = ? ORDER BY nama_kelas ASC', [tahun_ajaran_id_induk]);
    return rows;
  },
};
