 ...
 * UPDATE: ✅ FIXED - JOIN ke tahun_ajaran_induk, bukan tahun_ajaran
...
const kelasModel = {
  ...
  async getAll(tahun_ajaran_induk = null) {
    if (tahun_ajaran_induk) {
  ...
  async getById(id) {
    const [rows] = await db.execute(
      `SELECT k.*, 
              ...
              COALESCE(u.nama_lengkap, NULL) AS wali_kelas_nama
       FROM kelas k
  ...
  async getByTahunAjaran(tahun_ajaran_induk) {
    if (!tahun_ajaran_induk) {
      throw new Error('tahun_ajaran_induk wajib diisi');
    }
    const [rows] = await db.execute(
      'SELECT * FROM kelas WHERE tahun_ajaran_id = ? ORDER BY nama_kelas ASC', 
      [tahun_ajaran_induk]
