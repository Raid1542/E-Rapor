 ...
 * Fungsi: Controller untuk ekstrakurikuler siswa
 */
...
/**
 * GET /ekskul
 * Ambil daftar ekstrakurikuler per siswa di kelas guru
 */
       ...
       ORDER BY s.nama_lengkap`,
        ...
        for (const siswa of siswaRows) {
            ...
            const [ekskulRows] = await db.execute(
                `SELECT e.id_ekskul, e.nama_ekskul, e.deskripsi 
        ...
        const [daftar_ekskul] = await db.execute(
            `SELECT id_ekskul, nama_ekskul, deskripsi 
...
/**
 * PUT /ekskul/:siswaId
 * Update daftar ekstrakurikuler siswa (maksimal 3 item)
 */
exports.updateEkskulSiswa = async (req, res) => {
    try {
        ...
        for (const ekskul of ekskulList) {
         ...
         (siswa_id, ekskul_id, id_tahun_ajaran_induk, deskripsi, created_at, updated_at) 
         VALUES (?, ?, ?, ?, NOW(), NOW()) 
