 ...
 * Fungsi: Controller dashboard guru kelas dengan progress penilaian.
 *         Menampilkan progress kokurikuler, akademik, absensi, dan ekskul.
...
/**
 * Helper untuk cek gap dalam range nilai.
 */
const checkRangeGaps = (rows, minExpected = 0, maxExpected = 100, minField = 'rentang_min', maxField = 'rentang_max') => {
    const gaps = [];
...
/**
 * Ambil data dashboard guru kelas.
 */
exports.getDashboardData = async (req, res) => {
    try {
        ...
        if (!userId || !idTahunAjaranInduk || !idSemesterAktif) {
            return res.status(400).json({
                success: false,
                message: 'Data konteks pengguna atau tahun ajaran tidak ditemukan.'
            });
        }
    ...
    `);
        ...
        if (jenisPenilaianAktif === 'PTS') {
            ...
            const [totalAspekResult] = await db.execute(`SELECT COUNT(*) as total_aspek FROM aspek_kokurikuler`);
        ...
        AND catatan_wali_kelas IS NOT NULL AND catatan_wali_kelas != ''`,
            [kelasInfo.id_kelas, semesterId, jenisPenilaianAktif || 'PTS']
        );
        const catatanSudah = catatanResult[0]?.sudah || 0;

        let ekskulSudah = 0;
        if (jenisPenilaianAktif === 'PAS') {
    ...
    `);
        ...
        if (jenisPenilaianAktif === 'PTS') {
            ...
            const [aspekRows] = await db.execute(`SELECT id_aspek_kokurikuler, nama FROM aspek_kokurikuler`);
        ...
        for (const mapel of mapelRows) {
            ...
            AND jenis_penilaian = ?
            ORDER BY min_nilai`,
                [mapel.id_mata_pelajaran, kelasInfo.id_kelas, semesterId, jenisPenilaianAktif || 'PTS']
            );
        ...
        } else if (jenisPenilaianAktif === 'PTS') {
            ...
            data: {
                ...
                progress_lainnya: {
                    ...
                    kokurikuler: {
                        ...
                        subtitle: jenisPenilaianAktif === 'PTS' ? "Mutaba'ah" : 'Semua Aspek'
                    },
                    catatan_wali_kelas: {
                        sudah: catatanSudah,
                        total: totalSiswa,
                    ...
                    ekskul: {
                        ...
                        tersedia: jenisPenilaianAktif === 'PAS'
                    }
                }
            }
        });
