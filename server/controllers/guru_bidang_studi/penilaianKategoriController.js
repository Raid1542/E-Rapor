...
// GET: Ambil daftar kategori nilai akademik untuk mapel dan kelas tertentu
exports.getKategoriAkademik = async (req, res) => {
    try {
        const { mapel_id, kelas_id } = req.query;

        // Validasi parameter
        if (!mapel_id || !kelas_id) return res.status(400).json({ success: false, message: 'Parameter mapel_id dan kelas_id wajib diisi' });
        ...
        if (!taAktif) return res.status(400).json({ success: false, message: 'Tidak ada semester aktif' });
        ...
        const kategori = await kategoriModel.getKategoriByMapel(mapelIdNum, semesterId, kelasIdNum, jenisPenilaianAktif);
        
        // Hitung coverage range
        const coverage = await kategoriModel.cekCoverage0to100(mapelIdNum, semesterId, kelasIdNum, jenisPenilaianAktif);
        console.log(`[Coverage Result]:`, JSON.stringify(coverage));

        res.json({ success: true, data: kategori, coverage, mapel: req.penugasanMapel?.nama_mapel || 'Mata Pelajaran', kelas_id: kelasIdNum, jenis_penilaian: jenisPenilaianAktif });
    } catch (err) {
...
// POST: Tambah kategori nilai akademik baru dengan validasi overlap
exports.createKategoriAkademik = async (req, res) => {
    try {
        ...
        if (isNaN(min_nilai) || isNaN(max_nilai)) return res.status(400).json({ success: false, message: 'Nilai min dan max harus berupa angka.' });
        if (min_nilai < 0 || max_nilai > 100) return res.status(400).json({ success: false, message: 'Nilai harus antara 0 dan 100.' });
        if (min_nilai > max_nilai) return res.status(400).json({ success: false, message: `Nilai minimum (${min_nilai}) tidak boleh lebih besar dari nilai maksimum (${max_nilai}).` });
        ...
        const overlaps = await kategoriModel.cekRangeOverlap(mapelIdNum, semesterId, min_nilai, max_nilai, kelasIdNum, null, jenisPenilaianAktif);
        if (overlaps.length > 0) {
            return res.status(400).json({ success: false, message: `Range nilai ${min_nilai}-${max_nilai} tumpang tindih dengan kategori: ${kategoriModel.formatOverlapInfo(overlaps)}` });
        }
        ...
        const result = await kategoriModel.createKategori({
            mapel_id: mapelIdNum, semester_id: semesterId, min_nilai, max_nilai, deskripsi, kelas_id: kelasIdNum, jenis_penilaian: jenisPenilaianAktif
        });

        res.json({ success: true, message: 'Kategori berhasil ditambahkan', id: result.insertId, mapel: req.penugasanMapel?.nama_mapel, jenis_penilaian: jenisPenilaianAktif });
    } catch (err) {
...
// PUT: Update kategori nilai akademik dengan validasi overlap
exports.updateKategoriAkademik = async (req, res) => {
    try {
        ...
        if (kategoriModel.isUnchanged(existing, { min_nilai, max_nilai, deskripsi })) return res.status(400).json({ success: false, message: 'Tidak ada perubahan data.' });

        // Cek overlap dengan filter jenis_penilaian (exclude diri sendiri)
        const overlaps = await kategoriModel.cekRangeOverlap(existing.mapel_id, semesterId, min_nilai, max_nilai, existing.kelas_id, id, jenisPenilaianAktif);
        if (overlaps.length > 0) {
            return res.status(400).json({ success: false, message: `Range nilai ${min_nilai}-${max_nilai} tumpang tindih dengan kategori: ${kategoriModel.formatOverlapInfo(overlaps)}` });
        }
        ...
        // Update kategori
        const affectedRows = await kategoriModel.updateKategori(id, { min_nilai, max_nilai, deskripsi, urutan });
        if (affectedRows === 0) return res.status(404).json({ success: false, message: 'Kategori tidak ditemukan' });

        res.json({ success: true, message: 'Kategori akademik berhasil diperbarui', mapel: req.penugasanMapel?.nama_mapel });
    } catch (err) {
...
// DELETE: Hapus kategori nilai akademik dengan validasi dependensi
exports.deleteKategoriAkademik = async (req, res) => {
    try {
        ...
        const affectedCount = await kategoriModel.cekNilaiSiswaInRange(kategori.mapel_id, semesterId, kategori.min_nilai, kategori.max_nilai, kategori.kelas_id);
        if (affectedCount > 0) {
            return res.status(400).json({ success: false, message: `Tidak dapat menghapus kategori. Ada ${affectedCount} nilai siswa yang menggunakan range ${kategori.min_nilai}-${kategori.max_nilai}.` });
        }
        ...
        res.json({ success: true, message: 'Kategori berhasil dihapus', mapel: req.penugasanMapel?.nama_mapel });
    } catch (err) {
