...
exports.getKelasByMapel = async (req, res) => {
    try {
        ...
        if (!semesterId) {
            return res.status(400).json({ success: false, message: 'Konteks tahun ajaran tidak ditemukan' });
        }
        ...
        if (rows.length === 0) {
            return res.status(403).json({ success: false, message: 'Anda tidak mengajar mapel ini di kelas manapun' });
        }
