 ...
 * Fungsi: Controller profil guru bidang studi (ambil, edit, password, foto)
...
// GET: Ambil data profil guru bidang studi lengkap
exports.getProfil = async (req, res) => {
    try {
        const userId = req.user.id;
        const [userRows] = await db.execute('SELECT id_user, nama_lengkap, email_sekolah FROM user WHERE id_user = ?', [userId]);
        const [guruRows] = await db.execute('SELECT niy, nuptk, jenis_kelamin, no_telepon, alamat, tempat_lahir, tanggal_lahir, foto_path FROM guru WHERE user_id = ?', [userId]);
        if (userRows.length === 0) return res.status(404).json({ success: false, message: 'User tidak ditemukan' });
        const user = {
            id: userRows[0].id_user, role: 'guru bidang studi', nama_lengkap: userRows[0].nama_lengkap, email_sekolah: userRows[0].email_sekolah,
            niy: guruRows[0]?.niy || null, nuptk: guruRows[0]?.nuptk || null, jenis_kelamin: guruRows[0]?.jenis_kelamin || null,
            no_telepon: guruRows[0]?.no_telepon || null, alamat: guruRows[0]?.alamat || null, tempat_lahir: guruRows[0]?.tempat_lahir || null,
            tanggal_lahir: guruRows[0]?.tanggal_lahir || null, profileImage: guruRows[0]?.foto_path || null
        };
...
// PUT: Update data profil guru bidang studi
exports.editProfil = async (req, res) => {
    try {
        const { nama_lengkap, email_sekolah, niy, nuptk, jenis_kelamin, no_telepon, alamat, tempat_lahir, tanggal_lahir } = req.body;
        if (!nama_lengkap || !email_sekolah) return res.status(400).json({ success: false, message: 'Nama dan email wajib diisi' });
        const userId = req.user.id;
        await db.execute('UPDATE user SET nama_lengkap = ?, email_sekolah = ? WHERE id_user = ?', [nama_lengkap, email_sekolah, userId]);
        await db.execute(
            'UPDATE guru SET niy = ?, nuptk = ?, jenis_kelamin = ?, no_telepon = ?, alamat = ?, tempat_lahir = ?, tanggal_lahir = ? WHERE user_id = ?',
            [niy || null, nuptk || null, jenis_kelamin || null, no_telepon || null, alamat || null, tempat_lahir || null, tanggal_lahir || null, userId]
        );
        const [userRows] = await db.execute('SELECT id_user, nama_lengkap, email_sekolah FROM user WHERE id_user = ?', [userId]);
        const [guruRows] = await db.execute('SELECT niy, nuptk, jenis_kelamin, no_telepon, alamat, tempat_lahir, tanggal_lahir, foto_path FROM guru WHERE user_id = ?', [userId]);
        const user = {
...
// PUT: Ganti password dengan verifikasi password lama
exports.gantiPassword = async (req, res) => {
    try {
        ...
        if (!isMatch) return res.status(400).json({ success: false, message: 'Kata sandi lama salah' });
...
// PUT: Upload foto profil guru
exports.uploadFotoProfil = async (req, res) => {
    try {
        if (!req.file) return res.status(400).json({ success: false, message: 'File foto diperlukan' });
        const userId = req.user.id;
