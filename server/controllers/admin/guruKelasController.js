...
/**
 * Ambil data wali kelas berdasarkan ID kelas.
 */
...
exports.setWaliKelas = async (req, res) => {
    ...
    try {
        ...
        if (guruResult.length === 0) {
            return res.status(404).json({ success: false, message: 'Guru tidak ditemukan atau bukan guru kelas aktif' });
        }
 ...
 */
exports.getGuruKelasList = async (req, res) => {
    try {
        const [rows] = await db.execute(
            `SELECT u.id_user AS user_id, u.nama_lengkap AS nama 
        FROM user u 
