 ...
 * UPDATE: ✅ FIXED - kelas.tahun_ajaran_id sekarang mereferensi tahun_ajaran_induk
...
const getKelas = async (req, res) => {
    try {
        ...
        // ✅ FIXED: Query langsung pakai idInduk (bukan subquery ke semester)
        const [rows] = await db.execute(`
...
const getKelasById = async (req, res) => {
    try {
        ...
        // ✅ FIXED: Cek status semester aktif
        const semesterAktif = await getIdSemesterAktif(tahunAjaranIdInduk);
        kelas.is_aktif = semesterAktif !== null;
...
const getKelasForDropdown = async (req, res) => {
    try {
        ...
        // ✅ FIXED: Query langsung pakai idInduk
        const [rows] = await db.execute(
...
const tambahKelas = async (req, res) => {
    ...
    try {
        ...
        // ✅ FIXED: Cek apakah ada semester aktif (untuk validasi saja)
        ...
        // ✅ FIXED: Gunakan idInduk langsung (bukan semester ID)
        const tahun_ajaran_id = idInduk;
        ...
        // ✅ FIXED: Create kelas dengan tahun_ajaran_id = idInduk
        ...
        if (user_id && Number(user_id) > 0) {
            await db.execute(
                ...
                [Number(user_id), id, semesterAktif]
            );
        }
...
const editKelas = async (req, res) => {
    try {
        ...
        // ✅ FIXED: Gunakan idInduk langsung
        const tahun_ajaran_id = idInduk;
        ...
        const isDuplicate = allKelas.some(k => 
            k.nama_kelas.toLowerCase().trim() === nama_kelas.toLowerCase().trim() && 
            k.id_kelas !== Number(id)
        );
        if (isDuplicate) {
            return res.status(400).json({ 
                success: false, 
        ...
        if (existingUserId !== newUserId) {
            ...
            // ✅ FIXED: Delete dan insert wali kelas (gunakan semester ID)
            if (semesterAktif) {
                await db.execute('DELETE FROM guru_kelas WHERE kelas_id = ? AND tahun_ajaran_id = ?', [id, semesterAktif]);
                
                if (newUserId && newUserId > 0) {
                    await db.execute(
                        'INSERT INTO guru_kelas (user_id, kelas_id, tahun_ajaran_id, created_at, updated_at) VALUES (?, ?, ?, NOW(), NOW())',
                        [newUserId, id, semesterAktif]
                    );
                }
            }
        }
...
const hapusKelas = async (req, res) => {
    try {
        ...
        // ✅ FIXED: Delete wali kelas (gunakan semester ID)
        const semesterAktif = await getIdSemesterAktif(idInduk);
        if (semesterAktif) {
            await db.execute('DELETE FROM guru_kelas WHERE kelas_id = ? AND tahun_ajaran_id = ?', [id, semesterAktif]);
        }
    ...
    try {
        ...
        if (semesterAktif) {
            ...
            if (user_id && Number(user_id) > 0) {
                await db.execute(
                    'INSERT INTO guru_kelas (user_id, kelas_id, tahun_ajaran_id, created_at, updated_at) VALUES (?, ?, ?, NOW(), NOW())',
                    [Number(user_id), Number(id), semesterAktif]
                );
            }
        }
