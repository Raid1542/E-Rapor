 ...
 * Fungsi: Controller untuk manajemen penugasan mengajar (pembelajaran).
 *         Menangani penugasan mapel wajib (bulk) dan mapel pilihan (individual),
 *         serta validasi semester aktif dan duplikasi penugasan.
 ...
 * @param {number} idInduk - ID tahun ajaran induk
 * @returns {number|null} ID tahun ajaran aktif atau null
 */
...
/**
 * GET /api/admin/pembelajaran
 * Ambil daftar semua penugasan mengajar berdasarkan semester.
 * 
 * @param {string} req.query.tahun_ajaran_id - ID semester
 */
    ...
    try {
        ...
        if (!tahun_ajaran_id || isNaN(Number(tahun_ajaran_id))) {
            return res.status(400).json({
                success: false,
                message: 'tahun_ajaran_id (semester_id) wajib diisi dan harus angka'
            });
        }
...
/**
 * GET /api/admin/pembelajaran/kelas/:kelasId
 * Ambil data pembelajaran per kelas dengan pemisahan mapel wajib dan pilihan.
 * 
 * Response includes:
 *   - Info kelas dan wali kelas
 *   - Mapel wajib yang sudah ditugaskan
 *   - Mapel pilihan yang sudah ditugaskan
 *   - Mapel yang belum ditugaskan (untuk dropdown)
 */
    ...
    try {
        ...
        const [kelasRows] = await db.execute(
            `SELECT 
                ...
                FROM kelas k
                INNER JOIN tahun_ajaran ta ON k.tahun_ajaran_id = ta.id_tahun_ajaran
                WHERE k.id_kelas = ?`,
            [Number(kelasId)]
        );
...
/**
 * GET /api/admin/pembelajaran/dropdown
 * Ambil data dropdown untuk form pembelajaran (guru, kelas, mapel).
 */
    ...
    try {
        ...
        if (!semester_id) {
            ...
            if (!idInduk) {
                return res.status(400).json({
                    success: false,
                    message: 'semester_id wajib diisi atau tidak ada tahun ajaran aktif'
                });
            }
...
/**
 * POST /api/admin/pembelajaran/mapel-wajib
 * Tugaskan mapel wajib ke kelas (bulk insert).
 * Semua mapel wajib otomatis ditugaskan ke wali kelas.
 * 
 * Validasi:
 *   - Semester harus aktif
 *   - Kelas harus memiliki wali kelas
 *   - Mapel harus jenis wajib
...
const tambahMapelWajib = async (req, res) => {
    ...
    try {
        ...
        if (!kelas_id || !mapel_ids || !Array.isArray(mapel_ids) || mapel_ids.length === 0) {
            return res.status(400).json({
                success: false,
                message: 'kelas_id dan mapel_ids (array) wajib diisi.'
            });
        }
        ...
        if (!semester_id) {
            return res.status(400).json({
                success: false,
                message: 'semester_id wajib diisi.'
            });
        }
        ...
        if (!validation.data.is_aktif) {
            return res.status(403).json({
                success: false,
                message: `Tidak dapat menambah mapel di semester ${validation.data.semester} yang tidak aktif.`
            });
        }
        ...
        if (!waliKelas) {
            return res.status(400).json({
                success: false,
                message: `Kelas ${kelasCheck[0].nama_kelas} belum memiliki wali kelas. Tetapkan wali kelas terlebih dahulu.`
            });
        }
        ...
        if (mapelCheck.length !== mapel_ids.length) {
            return res.status(400).json({
                success: false,
                message: 'Beberapa mata pelajaran tidak valid atau bukan milik semester yang dipilih.'
            });
        }
        ...
        if (nonWajib.length > 0) {
            return res.status(400).json({
                success: false,
                message: `"${nonWajib.map(m => m.nama_mapel).join(', ')}" bukan mata pelajaran wajib.`
            });
        }
        ...
        res.status(201).json({
            success: true,
            message: `${inserted.length} mata pelajaran wajib berhasil ditugaskan ke ${waliKelas.nama_lengkap} di kelas ${kelasCheck[0].nama_kelas}.`,
        ...
        res.status(500).json({
            success: false,
            message: err.message || 'Gagal menambah mata pelajaran wajib'
        });
    } finally {
        connection.release();
    }
...
/**
 * POST /api/admin/pembelajaran/mapel-pilihan
 * Tugaskan mapel pilihan ke kelas dengan guru pengampu tertentu.
 * 
 * Validasi:
 *   - Semester harus aktif
 *   - Mapel harus jenis pilihan
 *   - Guru harus aktif
 *   - Tidak boleh duplikat mapel di kelas yang sama
 */
const tambahMapelPilihan = async (req, res) => {
    ...
    try {
        ...
        if (!kelas_id || !mapel_id || !user_id) {
            return res.status(400).json({
                success: false,
                message: 'Kelas, mata pelajaran, dan guru pengampu wajib diisi.'
            });
        }
        ...
        if (mapelCheck[0].jenis !== 'pilihan') {
            return res.status(400).json({
                success: false,
                message: `"${mapelCheck[0].nama_mapel}" bukan mata pelajaran pilihan.`
            });
        }
        ...
        if (isMapelDuplicate) {
            return res.status(400).json({
                success: false,
                message: `Mata pelajaran "${mapelCheck[0].nama_mapel}" sudah ditugaskan di kelas ini.`
            });
        }
        ...
        res.status(201).json({
            success: true,
            message: `Mata pelajaran pilihan "${mapelCheck[0].nama_mapel}" berhasil ditugaskan ke ${guruCheck[0].nama_lengkap} di kelas ${kelasCheck[0].nama_kelas}.`,
            id
        });
        ...
        if (err.code === 'ER_DUP_ENTRY' || err.errno === 1062) {
            return res.status(400).json({
                success: false,
                message: 'Kombinasi ini sudah ada.'
            });
        }

        res.status(500).json({
            success: false,
            message: err.message || 'Gagal menambah mata pelajaran pilihan'
        });
    } finally {
        connection.release();
    }
...
/**
 * POST /api/admin/pembelajaran
 * Tambah penugasan mengajar (legacy endpoint).
 * Digunakan untuk kompatibilitas dengan versi sebelumnya.
 */
    ...
    try {
        ...
        if (semester_id) {
            ...
            if (!validation.data.is_aktif) {
                return res.status(403).json({
                    success: false,
                    message: `Semester ${validation.data.semester} tidak aktif.`
                });
            }
        ...
        const id = await pembelajaranModel.create(
            { tahun_ajaran_id: taId, kelas_id: kelasIdNum, mapel_id: mapelIdNum, user_id: userIdNum },
            connection
        );
...
/**
 * PUT /api/admin/pembelajaran/:id
 * Update penugasan mengajar (ganti guru/kelas/mapel).
 */
const editPembelajaran = async (req, res) => {
    ...
    try {
        ...
        const success = await pembelajaranModel.update(
            idNum, 
            { kelas_id: kelasIdNum, mapel_id: mapelIdNum, user_id: userIdNum }, 
            connection
        );
...
/**
 * DELETE /api/admin/pembelajaran/:id
 * Hapus penugasan mengajar dengan validasi dependensi nilai rapor.
 */
const hapusPembelajaran = async (req, res) => {
    try {
        ...
        if (jumlahNilai > 0) {
            return res.status(400).json({
                success: false,
                message: `Tidak bisa dihapus: sudah ada ${jumlahNilai} data nilai rapor.`
            });
        }
