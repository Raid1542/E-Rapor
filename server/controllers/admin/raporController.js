...
/**
 * Ambil data rapor lengkap untuk satu siswa.
 */
 ...
 */
exports.getTahunAjaranAll = async (req, res) => {
    try {
 ...
 */
exports.getKelasByTahunAjaran = async (req, res) => {
    try {
        ...
        const [rows] = await db.execute(
            'SELECT id_kelas, nama_kelas FROM kelas WHERE tahun_ajaran_id = ? ORDER BY nama_kelas',
            [idInduk]
        );
 ...
 */
exports.getDaftarSiswaUntukRapor = async (req, res) => {
    try {
...
/**
 * Atur status penilaian PTS atau PAS dengan validasi business logic.
 */
exports.aturStatusPenilaian = async (req, res) => {
    try {
        ...
        const actionText = status === 'aktif'
            ? 'menginput nilai'
            : status === 'selesai'
                ? 'mengunduh rapor (data terkunci)'
                : 'melihat nilai (tidak dapat mengedit)';

        res.json({
            success: true,
            message: `Status ${jenis} Semester ${semester} berhasil diubah menjadi "${status.toUpperCase()}". Guru sekarang dapat ${actionText}.`,
            data: {
                jenis,
                status,
                semester,
...
/**
 * Arsipkan rapor semua siswa dan kunci status menjadi selesai.
 */
exports.arsipkanRapor = async (req, res) => {
    try {
        ...
        const [siswaList] = await db.execute(
            'SELECT sk.siswa_id, sk.kelas_id FROM siswa_kelas sk WHERE sk.id_tahun_ajaran_induk = ?',
            [tahun_ajaran_id]
        );
