...
// GET: Ambil daftar semua siswa dengan pagination dan filter status
...
// GET: Ambil detail siswa berdasarkan ID
...
// POST: Tambah siswa baru dengan validasi duplikasi NIS/NISN
const tambahSiswaMaster = async (req, res) => {
    try {
        ...
        if (!nis || !nama_lengkap || !jenis_kelamin) return res.status(400).json({ success: false, message: 'NIS, nama lengkap, dan jenis kelamin wajib diisi' });
        ...
        if (nisExists) return res.status(400).json({ success: false, message: `NIS "${trimmedNis}" sudah digunakan`, code: 'DUPLICATE_NIS' });

        // Cek duplikat NISN (jika ada)
        if (trimmedNisn) {
            const nisnExists = await SiswaModel.checkNisnExists(trimmedNisn);
            if (nisnExists) return res.status(400).json({ success: false, message: `NISN "${trimmedNisn}" sudah digunakan`, code: 'DUPLICATE_NISN' });
        }

        // Cek nama sama (warning, bukan error)
        const namaExists = await SiswaModel.checkNamaExists(trimmedNama);
        let warningMessage = null;
        if (namaExists) warningMessage = `Perhatian: Sudah ada siswa dengan nama "${trimmedNama}" di sistem.`;

        // Insert data
        const id = await SiswaModel.createSiswa({
            nis: trimmedNis, nisn: trimmedNisn, nama_lengkap: trimmedNama,
            tempat_lahir: tempat_lahir ? tempat_lahir.trim() : null, tanggal_lahir: tanggal_lahir || null,
            jenis_kelamin: jenis_kelamin, alamat: alamat ? alamat.trim() : null
        });

        res.status(201).json({ success: true, message: 'Siswa berhasil ditambahkan', id: id, warning: warningMessage });
    } catch (err) {
        console.error('Error tambahSiswaMaster:', err);
        if (err.code === 'ER_DUP_ENTRY' || err.errno === 1062) {
            return res.status(400).json({ success: false, message: 'NIS atau NISN sudah terdaftar di sistem!', code: 'DUPLICATE_ENTRY' });
        }
        res.status(500).json({ success: false, message: 'Gagal menambah siswa: ' + err.message });
    }
};

// PUT: Update data siswa dengan validasi duplikasi (exclude diri sendiri)
const editSiswaMaster = async (req, res) => {
    try {
        ...
        if (!existingSiswa) return res.status(404).json({ success: false, message: 'Siswa tidak ditemukan' });
        ...
        const updated = await SiswaModel.updateSiswa(id, {
            nis: trimmedNis, nisn: trimmedNisn, nama_lengkap: trimmedNama,
            tempat_lahir: tempat_lahir ? tempat_lahir.trim() : null, tanggal_lahir: tanggal_lahir || null,
            jenis_kelamin: jenis_kelamin, alamat: alamat ? alamat.trim() : null, status: status || 'aktif'
        });

        if (!updated) return res.status(404).json({ success: false, message: 'Gagal memperbarui data siswa' });
...
// DELETE: Hapus siswa dengan soft delete (ubah status jadi nonaktif)
// Validasi: siswa tidak boleh masih terdaftar di kelas manapun
const hapusSiswaMaster = async (req, res) => {
    try {
        ...
        if (!deleted) return res.status(404).json({ success: false, message: 'Gagal menghapus siswa' });

        res.json({ success: true, message: `Siswa "${existingSiswa.nama_lengkap}" berhasil dihapus (soft delete)` });
    } catch (err) {
...
// POST: Import data siswa dari file Excel (.xlsx)
// Fitur: skip duplikat, auto-convert tanggal Excel, return list yang di-skip
    ...
    try {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            if (!row.nis || !row.nama_lengkap || !row.jenis_kelamin) {
                skipped.push({ row: rowNumber, nama: row.nama_lengkap || '-', reason: 'Kolom wajib (NIS, nama lengkap, jenis kelamin) tidak lengkap' });
                continue;
            }
            ...
            const nisExists = await SiswaModel.checkNisExists(trimmedNis);
            if (nisExists) {
                skipped.push({ row: rowNumber, nama: trimmedNama, reason: `NIS "${trimmedNis}" sudah terdaftar` });
                continue;
            }
            ...
            if (trimmedNisn) {
                const nisnExists = await SiswaModel.checkNisnExists(trimmedNisn);
                if (nisnExists) {
                    skipped.push({ row: rowNumber, nama: trimmedNama, reason: `NISN "${trimmedNisn}" sudah terdaftar` });
                    continue;
                }
            }
            ...
            try {
                await SiswaModel.createSiswa({
                    ...
                    tanggal_lahir: tanggal_lahir, jenis_kelamin: row.jenis_kelamin || 'Laki-laki',
                    alamat: row.alamat ? String(row.alamat).trim() : null
                });
                processedCount++;
            } catch (insertErr) {
                if (insertErr.code === 'ER_DUP_ENTRY' || insertErr.errno === 1062) {
                    skipped.push({ row: rowNumber, nama: trimmedNama, reason: `NIS "${trimmedNis}" atau NISN "${trimmedNisn}" sudah terdaftar` });
                } else {
        ...
        res.json({
            success: true,
            message: skipped.length > 0 ? `Import selesai: ${processedCount} berhasil, ${skipped.length} dilewati` : `Import berhasil: ${processedCount} siswa ditambahkan`,
            total: processedCount, skipped: skipped
        });
    } catch (err) {
...
module.exports = { getSiswaMaster, getSiswaMasterById, tambahSiswaMaster, editSiswaMaster, hapusSiswaMaster, importSiswaMaster };