...
// Komponen Popup: modal notifikasi dengan animasi smooth (success/error/warning)
const LoginPopup = ({ popup, onClose }: { popup: PopupConfig; onClose: () => void }) => {
    ...
    const cfg = {
        success: {
            ...
            btnBg: 'rgba(255,255,255,0.22)', btnBorder: 'rgba(255,255,255,0.4)', btnColor: '#fff', btnHoverBg: 'rgba(255,255,255,0.32)',
            deco1: 'rgba(255,255,255,0.08)', deco2: 'rgba(255,255,255,0.05)',
        },
        error: {
            bg: 'linear-gradient(135deg, #991b1b 0%, #dc2626 55%, #ef4444 100%)',
            iconBg: 'rgba(255,255,255,0.15)', iconBorder: 'rgba(255,255,255,0.3)',
            icon: (<svg width="44" height="44" viewBox="0 0 44 44" fill="none"><circle cx="22" cy="22" r="22" fill="rgba(255,255,255,0.12)" /><path d="M15 15l14 14M29 15L15 29" stroke="white" strokeWidth="3" strokeLinecap="round" /></svg>),
            titleColor: '#fff', msgColor: 'rgba(254,226,226,0.9)',
            btnBg: 'rgba(255,255,255,0.18)', btnBorder: 'rgba(255,255,255,0.35)', btnColor: '#fff', btnHoverBg: 'rgba(255,255,255,0.28)',
            deco1: 'rgba(255,255,255,0.07)', deco2: 'rgba(255,255,255,0.04)',
        },
        warning: {
            bg: 'linear-gradient(135deg, #92400e 0%, #d97706 55%, #f59e0b 100%)',
            iconBg: 'rgba(255,255,255,0.15)', iconBorder: 'rgba(255,255,255,0.3)',
            icon: (<svg width="44" height="44" viewBox="0 0 44 44" fill="none"><circle cx="22" cy="22" r="22" fill="rgba(255,255,255,0.12)" /><path d="M22 14v10M22 28v2" stroke="white" strokeWidth="3" strokeLinecap="round" /></svg>),
            titleColor: '#fff', msgColor: 'rgba(254,243,199,0.9)',
            btnBg: 'rgba(255,255,255,0.18)', btnBorder: 'rgba(255,255,255,0.35)', btnColor: '#fff', btnHoverBg: 'rgba(255,255,255,0.28)',
            deco1: 'rgba(255,255,255,0.07)', deco2: 'rgba(255,255,255,0.04)',
        },
    }[popup.type];
    ...
    return (
        <>
            ...
            <div className="lp-backdrop" style={{ position: 'fixed', inset: 0, zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px', background: 'rgba(12,5,2,0.62)', backdropFilter: 'blur(6px)', WebkitBackdropFilter: 'blur(6px)' }} onClick={e => { if (e.target === e.currentTarget) handleClose(); }}>
                <div className="lp-card" style={{ position: 'relative', width: '100%', maxWidth: '360px', borderRadius: '24px', overflow: 'hidden', background: cfg.bg, boxShadow: '0 32px 80px rgba(0,0,0,0.42), 0 0 0 1px rgba(255,255,255,0.1)', padding: '44px 32px 36px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0', textAlign: 'center' }}>
                    <div className="lp-deco1" style={{ background: cfg.deco1 }} />
                    <div className="lp-deco2" style={{ background: cfg.deco2 }} />

                    <div className="lp-icon" style={{ width: 80, height: 80, borderRadius: '50%', background: cfg.iconBg, border: `2.5px solid ${cfg.iconBorder}`, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '20px', position: 'relative', zIndex: 1, flexShrink: 0 }}>
                        {cfg.icon}
                    </div>

                    <p className="lp-label-shimmer" style={{ fontSize: '10px', fontWeight: 700, letterSpacing: '0.18em', marginBottom: '8px', position: 'relative', zIndex: 1 }}>{label}</p>

                    <h3 style={{ color: cfg.titleColor, fontSize: '22px', fontWeight: 800, lineHeight: 1.25, margin: '0 0 10px', position: 'relative', zIndex: 1 }}>{popup.title}</h3>

                    <div style={{ width: 40, height: 2.5, background: 'rgba(255,255,255,0.35)', borderRadius: 2, margin: '0 0 14px', position: 'relative', zIndex: 1 }} />

                    <p style={{ color: cfg.msgColor, fontSize: '13.5px', lineHeight: 1.65, margin: '0 0 28px', position: 'relative', zIndex: 1, fontWeight: 400 }}>{popup.message}</p>

                    {popup.type === 'success' && (
                        <div className="lp-dots" style={{ display: 'flex', gap: 6, marginBottom: 20, position: 'relative', zIndex: 1 }}>
                            {[0, 1, 2].map(i => (<div key={i} style={{ width: 7, height: 7, borderRadius: '50%', background: 'rgba(255,255,255,0.7)', animationDelay: `${i * 0.2}s` }} />))}
                        </div>
                    )}

                    {popup.type !== 'success' && (
                        <button className="lp-btn" onClick={handleClose} style={{ width: '100%', padding: '13px 0', borderRadius: '13px', border: `1.5px solid ${cfg.btnBorder}`, background: cfg.btnBg, color: cfg.btnColor, fontSize: '14px', fontWeight: 700, cursor: 'pointer', letterSpacing: '0.04em', position: 'relative', zIndex: 1, fontFamily: 'Poppins, sans-serif' }} onMouseEnter={e => (e.currentTarget.style.background = cfg.btnHoverBg)} onMouseLeave={e => (e.currentTarget.style.background = cfg.btnBg)}>
                            Coba Lagi
                        </button>
                    )}
                </div>
...
// Komponen utama: form login + fetch data sekolah publik + handle submit + redirect
export default function LoginClient() {
    ...
    // Fetch data sekolah publik (nama + logo) saat mount
    useEffect(() => {
    ...
    // Handle submit: validasi form → POST /api/auth/login → simpan token/user → redirect
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!email_sekolah.trim() || !password || !role) {
            showPopup({ type: 'warning', title: 'Form Belum Lengkap', message: 'Email, password, dan role wajib diisi sebelum melanjutkan.' });
            return;
        }
        ...
        try {
            ...
            if (data.user) {
                const userData = { ...data.user, role: role, profileImage: data.user.profileImage || data.user.foto_path || null };
            ...
            // Ambil redirect parameter atau default dashboard
            const urlParams = new URLSearchParams(window.location.search);
            const redirect = urlParams.get('redirect');
            const defaultDashboard = role === "admin" ? "/admin/dashboard" : role === "guru_kelas" ? "/guru_kelas/dashboard" : "/guru_bidang_studi/dashboard";
            const redirectPath = redirect || defaultDashboard;

            showPopup({ type: 'success', title: 'Login Berhasil!', message: `Selamat datang kembali. Anda masuk sebagai ${roleLabel}. Mengalihkan ke dashboard...` });

            // Hard redirect setelah 1.5 detik
            setTimeout(() => {
            ...
            setLoading(false);
            showPopup({ type: 'error', title: 'Koneksi Gagal', message: 'Tidak dapat terhubung ke server. Periksa koneksi internet Anda dan coba lagi.' });
        }
    };

    // Handle input change (trim email)
                    ...
                    <div className="login-card">
                        ...
                        <div className="left-panel">
                            ...
                            <div className="left-text">
                                ...
                                <p className="brand-sub">Platform pengelolaan nilai dan<br />rapor siswa terintegrasi.</p>
                            </div>
                        </div>
                                    ...
                                    {loading ? (
                                        ...
                                        <>
                                            ...
                                            <div className="btn-arrow">
                                                <svg width="14" height="14" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 12h14M12 5l7 7-7 7" /></svg>
                                            </div>
                                        </>
                                    )}
                                </button>
