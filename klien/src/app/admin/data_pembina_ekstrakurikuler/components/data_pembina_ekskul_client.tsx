...
import { Eye, Pencil, Upload, X, Plus, Search, Filter, CheckCircle2, AlertCircle, WifiOff, ShieldAlert, ChevronLeft, User, } from 'lucide-react';
...
export default function DataPembinaEkskulClient() {
        ...
        try {
            ...
            if (res.ok) {
                ...
                if (result.skipped && result.skipped.length > 0) {
                    ...
                    const summaryLines = [
                        `Berhasil: ${result.total} pembina`, `Dilewati: ${skippedCount} pembina`, '',
    ...
    const filteredPembina = pembinaList.filter((pembina) => {
        ...
        const matchesSearch = !query || pembina.nama?.toLowerCase().includes(query) || pembina.niy?.includes(query) || pembina.nuptk?.includes(query);
        const matchesGender = !filterValues.jenisKelamin || pembina.jenis_kelamin?.toLowerCase() === filterValues.jenisKelamin.toLowerCase();
        const matchesStatus = !filterValues.status || pembina.status?.toLowerCase() === filterValues.status.toLowerCase();
    ...
    const renderForm = (isEdit: boolean) => (
        <div className={`flex-1 p-6 min-h-screen transition-all duration-300 ${formClosing ? 'opacity-0 translate-y-4' : 'opacity-100 translate-y-0'}`} style={PAGE_BG}>
            ...
            <div className="mb-6 flex items-center gap-3">
                ...
                <div>
                    <h1 className="text-2xl font-bold text-gray-900">{isEdit ? 'Edit Data Pembina' : 'Tambah Data Pembina'}</h1>
                    <p className="text-sm mt-0.5" style={{ color: '#c95b08' }}>{isEdit ? 'Perbarui informasi data pembina' : 'Isi formulir untuk menambahkan pembina baru'}</p>
                </div>
            </div>

            <div className="section-card bg-white rounded-2xl overflow-hidden" style={CARD_STYLE}>
                <div className="px-6 py-5 flex items-center gap-3" style={HEADER_GRAD}>
                    <div className="w-10 h-10 rounded-xl bg-white/20 flex items-center justify-center flex-shrink-0">
                        <User size={20} className="text-white" />
                    </div>
                    <div>
                        <p className="text-[11px] font-semibold uppercase tracking-wider text-white/75">{isEdit ? 'Formulir Edit' : 'Formulir Tambah'}</p>
                        <h2 className="text-base font-bold text-white leading-tight">{isEdit ? 'Ubah Data Pembina' : 'Data Pembina Baru'}</h2>
                    </div>
                </div>
                ...
                <div className="p-6 md:p-8 grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div className="md:col-span-2 flex justify-end gap-3 pt-5 mt-2" style={{ borderTop: '1px solid #fde0c8' }}>
                        ...
                        <button onClick={() => openConfirmModal(isEdit ? 'edit' : 'add')} className={`btn-primary ${btnPrimary.base}`}
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 dg-fadeIn" onClick={(e) => { if (e.target === e.currentTarget) setShowConfirmModal(false); }}>
                    ...
                    <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg p-6 dg-scaleIn">
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <h3 className="text-base font-bold text-gray-900 whitespace-nowrap">Konfirmasi {confirmAction === 'add' ? 'Penambahan' : 'Perubahan'} Data</h3>
                        </div>
                        ...
                        <div className="flex gap-3">
                            ...
                            <button onClick={() => { setShowConfirmModal(false); confirmAction === 'add' ? executeTambah() : executeEdit(); }}
    ...
    return (
        <div className="flex-1 min-h-screen p-6" style={PAGE_BG}>
            ...
            <div className="section-card bg-white rounded-2xl px-5 py-3.5 mb-5 flex flex-wrap items-center justify-between gap-3" style={CARD_STYLE}>
                ...
                <div className="flex flex-wrap items-center gap-2">
                    <div className="flex items-center gap-2 whitespace-nowrap">
                        ...
                        <select value={itemsPerPage} onChange={(e) => { setItemsPerPage(Number(e.target.value)); setCurrentPage(1); }} className="border rounded-xl px-3 py-1.5 text-sm outline-none transition-all focus:ring-2 focus:ring-orange-400 focus:border-orange-400 bg-orange-50/40 border-orange-200">
                    ...
                    <div className="relative min-w-[200px] sm:min-w-[220px]">
                        ...
                        <input type="text" placeholder="Cari pembina..." value={searchQuery} onChange={(e) => { setSearchQuery(e.target.value); setCurrentPage(1); }}
                    ...
                    <button onClick={openFilterModal} className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold border transition-all"
                        style={{ background: '#fff', border: '1.5px solid #d97706', color: '#b35a08', boxShadow: '0 1px 4px rgba(217,119,6,0.15)' }}
                        onMouseEnter={e => (e.currentTarget.style.background = '#fff0e5')} onMouseLeave={e => (e.currentTarget.style.background = '#fff')}>
                        <Filter size={15} /> Filter
                    </button>

                    <button onClick={() => setShowImport(true)} className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold border transition-all"
            ...
            <div className="section-card bg-white rounded-2xl overflow-hidden" style={CARD_STYLE}>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full min-w-[800px] text-sm border-collapse">
                        <thead>
                            <tr style={TH_GRAD}>
                                {['No.', 'Nama', 'NIY', 'NUPTK', 'Jenis Kelamin', 'No. Telepon', 'Status', 'Aksi'].map(h => (
                        ...
                        <tbody>
                            {loading ? (
                                ...
                                <tr><td colSpan={8} className="py-12 text-center text-gray-400 text-sm">Tidak ada data pembina</td></tr>
                            ) : filteredPembina.map((pembina, index) => (
                                <tr key={pembina.id} className="item-hover transition-colors"
                                    ...
                                    <td className="px-5 py-3.5 text-center text-gray-700">{formatGender(pembina.jenis_kelamin)}</td>
                                    <td className="px-5 py-3.5 text-center text-gray-600">{pembina.no_telepon || '-'}</td>
                                    <td className="px-5 py-3.5 text-center">
                                        {pembina.status === 'aktif' ? (
                                            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[11px] font-bold" style={{ background: '#eaf7ef', color: '#1a7a3a', border: '1px solid #b6e8c8' }}>
                                                <span className="w-1.5 h-1.5 rounded-full bg-green-500 inline-block" />AKTIF
                                            </span>
                                        ) : (
                                            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[11px] font-bold" style={{ background: '#f3f4f5', color: '#6b7280', border: '1px solid #d1d5db' }}>
                                    ...
                                    <td className="px-5 py-3.5 text-center whitespace-nowrap">
                                        <div className="flex justify-center gap-2">
                                            <button onClick={() => handleDetail(pembina)} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all"
                                                style={{ background: '#eaf7ef', border: '1px solid #b6e8c8', color: '#1a7a3a' }}
                                                onMouseEnter={e => (e.currentTarget.style.background = '#d4f0de')} onMouseLeave={e => (e.currentTarget.style.background = '#eaf7ef')}>
                                                <Eye size={13} /> Detail
                                            </button>
                                            <button onClick={() => handleEdit(pembina)} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all"
                                                style={{ background: '#fff0e5', border: '1px solid #f5a623', color: '#b35a08' }}
                                                onMouseEnter={e => (e.currentTarget.style.background = '#ffe4c8')} onMouseLeave={e => (e.currentTarget.style.background = '#fff0e5')}>
                                                <Pencil size={13} /> Edit
                                            </button>
                                        </div>
                                    </td>
            ...
            {showDetail && selectedPembina && (
                <div className={`fixed inset-0 flex items-center justify-center z-50 p-3 sm:p-4 transition-opacity duration-200 ${detailClosing ? 'opacity-0' : 'opacity-100'}`} onClick={e => { if (e.target === e.currentTarget) closeDetail(); }}>
                    <div className="absolute inset-0 bg-black/50 backdrop-blur-sm" />
                    <div className={`relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[88vh] overflow-y-auto transform transition-all duration-200 ${detailClosing ? 'opacity-0 scale-95' : 'opacity-100 scale-100'}`} style={CARD_STYLE}>
                        ...
                        <div className="p-6">
                            ...
                            <div className="space-y-2.5">
                                {[
                                    { label: 'Status', value: <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold" style={selectedPembina.status === 'aktif' ? { background: '#eaf7ef', color: '#1a7a3a', border: '1px solid #b6e8c8' } : { background: '#f3f4f5', color: '#6b7280', border: '1px solid #d1d5db' }}>{selectedPembina.status?.toUpperCase() || 'AKTIF'}</span> },
                                    { label: 'NIY', value: selectedPembina.niy || '-' },
                                    { label: 'NUPTK', value: selectedPembina.nuptk || '-' },
                                    { label: 'Jenis Kelamin', value: formatGender(selectedPembina.jenis_kelamin) },
                            ...
                            <div className="flex justify-end gap-3 mt-6 pt-4" style={{ borderTop: '1px solid #fde0c8' }}>
                                ...
                                <button onClick={() => { handleEdit(selectedPembina); closeDetail(); }} className={`btn-primary ${btnPrimary.base}`} style={btnPrimary.style} onMouseEnter={btnPrimary.hover} onMouseLeave={btnPrimary.leave}>
                                    <Pencil size={14} /> Edit
                                </button>
                            </div>
                        </div>
            ...
            {/* ── Modal Import ─────────────────────────────────────────────────── */}
            {showImport && (
                <div className={`fixed inset-0 flex items-center justify-center z-50 p-3 sm:p-4 transition-opacity duration-200 ${importClosing ? 'opacity-0' : 'opacity-100'}`} onClick={e => { if (e.target === e.currentTarget) closeImport(); }}>
                    <div className="absolute inset-0 bg-black/50 backdrop-blur-sm" />
                    <div className={`relative bg-white rounded-2xl shadow-2xl w-full max-w-md transform transition-all duration-200 ${importClosing ? 'opacity-0 scale-95' : 'opacity-100 scale-100'}`} style={CARD_STYLE}>
                        ...
                        <div className="p-6">
                            ...
                            <div className="mb-4">
                                <a href="http://localhost:5000/templates/template_import_pembina.xlsx" download className="text-sm font-semibold flex items-center gap-1 hover:underline" style={{ color: '#e8690a' }}>
                            ...
                            <label className="flex flex-col items-center justify-center w-full h-32 rounded-xl cursor-pointer transition-colors"
                                ...
                                onMouseEnter={e => (e.currentTarget.style.background = '#fff0e5')} onMouseLeave={e => (e.currentTarget.style.background = '#fffaf6')}>
            ...
            {/* ── Modal Filter ─────────────────────────────────────────────────── */}
            {showFilter && (
                <div className={`fixed inset-0 flex items-center justify-center z-50 p-3 sm:p-4 transition-opacity duration-200 ${filterClosing ? 'opacity-0' : 'opacity-100'}`} onClick={e => { if (e.target === e.currentTarget) closeFilterModal(); }}>
                    <div className="absolute inset-0 bg-black/50 backdrop-blur-sm" />
                    <div className={`relative bg-white rounded-2xl shadow-2xl w-full max-w-sm transform transition-all duration-200 ${filterClosing ? 'opacity-0 scale-95' : 'opacity-100 scale-100'}`} style={CARD_STYLE}>
                        <div className="flex items-center justify-between px-6 py-4 rounded-t-2xl" style={HEADER_GRAD}>
                            <div className="flex items-center gap-2">
                                <h2 className="text-base font-bold text-white">Filter Pembina Ekstrakurikuler</h2>
                            </div>
                        ...
                        <div className="p-6 space-y-4">
                            ...
                            <div className="pt-2 flex gap-3" style={{ borderTop: '1px solid #fde0c8' }}>
                                ...
                                <button onClick={applyFilter} className="flex-1 py-2.5 rounded-xl text-sm font-bold text-white transition-all"
