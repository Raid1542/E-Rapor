...
export default function Sidebar({ user }: SidebarProps) {
  ...
  const [openDropdowns, setOpenDropdowns] = useState({ pengguna: false, administrasi: false });
  ...
  // ── Nav item styles — sama persis dengan Sidebar guru kelas ───────────────
  ...
  return (
    <div
      ...
      style={{ background: 'linear-gradient(175deg, #9a3a08 0%, #c95b08 40%, #e8690a 75%, #f5870a 100%)' }}
    >
          ...
          <>
            <div className="flex items-center gap-3 min-w-0">
              <div className="flex-shrink-0 w-12 h-12 rounded-xl flex items-center justify-center overflow-hidden"
                ...
                <img src={logoUrl} alt="Logo Sekolah" className="w-10 h-10 object-contain"
      ...
      <div className="flex-1 overflow-y-auto px-3 py-4 scrollbar-none">
        ...
        {/* Tahun Ajaran */}
        <button onClick={() => handleNavigation('/admin/data_tahun_ajaran')}
        ...
        <div className="mb-1">
          ...
          {isExpanded && openDropdowns.pengguna && (
            <div className="ml-4 mt-1 pl-3 py-1 space-y-0.5"
              style={{ borderLeft: '2px solid rgba(255,255,255,0.25)' }}>
              {penggunaSubmenu.map((item, idx) => (
                <button key={idx} onClick={() => handleNavigation(item.url)}
        ...
        <div className="mb-1">
          <button onClick={() => toggleDropdown('administrasi')}
        ...
        {/* Arsip Rapor */}
        <button onClick={() => handleNavigation('/admin/arsip_rapor')}
        ...
        {/* Backup & Restore */}
        <button onClick={() => handleNavigation('/admin/backup_restore')}
        ...
        {/* Profil */}
        <button onClick={() => handleNavigation('/admin/profil')}
      ...
      <div className="px-4 py-3" style={{ borderTop: '1px solid rgba(255,255,255,0.12)' }}>
        {isExpanded ? (
          ...
          <div className="w-9 h-9 rounded-full flex items-center justify-center mx-auto text-xs font-bold text-white"
            style={{ background: 'rgba(255,255,255,0.2)' }}>
            {user.nama_lengkap?.charAt(0)?.toUpperCase() ?? 'A'}
          </div>
        )}
