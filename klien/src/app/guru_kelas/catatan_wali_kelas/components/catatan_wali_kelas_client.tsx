...
// terpisah seperti sebelumnya yang tampilannya tidak konsisten).
// Tidak ada perubahan pada logika/validasi — hanya bentuk tampilannya.
...
export default function CatatanWaliClient() {
    ...
    const openConfirmModal = () => {
        ...
        showModal({
            ...
            message: 'Apakah Anda yakin ingin menyimpan catatan wali kelas ini?',
    ...
    const executeImport = async () => {
        ...
        try {
            ...
            if (data.data?.nis_duplikat_count && data.data.nis_duplikat_count > 0) {
                successMessage += `\n\nDitemukan ${data.data.nis_duplikat_count} NIS duplikat. Hanya data pertama yang diproses.`;
            }
    ...
    const NaikTingkatBadge = ({ value }: { value: 'ya' | 'tidak' | null }) => {
        if (value === 'ya') return <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-bold" style={{ background: '#dcfce7', color: '#166534', border: '1px solid #86efac' }}><CheckCircle2 size={12} /> Ya</span>;
        if (value === 'tidak') return <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-bold" style={{ background: '#fee2e2', color: '#991b1b', border: '1px solid #fca5a5' }}><AlertCircle size={12} /> Tidak</span>;
        return <span className="inline-flex items-center gap-1 px-2 py-1 rounded-md text-xs font-medium" style={{ background: '#fff5eb', color: ACCENT_DARK, border: '1px solid #f0a94e' }}><AlertCircle size={10} /> Belum diisi</span>;
    };

    // Grid kolom tabel — kolom Naik Tingkat hanya muncul saat PAS Genap
    const GRID_COLS = [
        'minmax(48px,0.5fr)',
        'minmax(180px,2.4fr)',
        'minmax(90px,0.9fr)',
            ...
            <div className="flex-1 min-h-screen p-6 flex items-center justify-center" style={PAGE_BG}>
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 dg-fadeIn">
                    ...
                    <div className="relative bg-white rounded-2xl w-full max-w-md p-8 flex flex-col items-center gap-4 dg-scaleIn" style={CARD_STYLE}>
                        <div className="w-20 h-20 rounded-full bg-red-50 flex items-center justify-center ring-8 ring-red-100 dg-pulse"><AlertCircle size={48} className="text-red-500" /></div>
    ...
    return (
        <div className="flex-1 min-h-screen p-3 sm:p-6" style={PAGE_BG}>
            ...
            <div className="mb-4 sm:mb-5 anim-in d1">
                <h1 className="text-xl sm:text-2xl font-bold text-gray-900">Catatan Wali Kelas</h1>
                <p className="text-xs sm:text-sm mt-1 text-gray-500">Kelola catatan perkembangan siswa kelas {kelasNama}</p>
            </div>
                    ...
                    <div className="flex items-center gap-3 px-5 py-3.5" style={{ background: readOnlyReason === 'locked' ? '#fee2e2' : '#fef3c7' }}>
                        ...
                        <div>
                            ...
                            <p className={`text-xs mt-0.5 ${readOnlyReason === 'locked' ? 'text-red-700' : 'text-yellow-700'}`}>
                                {readOnlyReason === 'locked' ? 'Periode penilaian telah selesai dan data sudah dikunci. Anda dapat melihat data siswa, tetapi tidak dapat mengedit catatan.' : 'Periode penilaian belum aktif. Anda dapat melihat data siswa, tetapi belum dapat menginput catatan. Silakan hubungi Administrator untuk membuka periode penilaian.'}
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <div className="card-flat bg-white rounded-2xl overflow-hidden mb-4 anim-in d2" style={CARD_STYLE}>
                ...
                {!isReadOnly && (
                    <div className="mx-4 sm:mx-5 my-5 flex items-start gap-3 px-4 py-3 rounded-xl" style={{ background: isPASGenap ? '#dcfce7' : '#fff5eb', border: `1px solid ${isPASGenap ? '#86efac' : '#f0a94e'}` }}>
                        <Info size={18} className="flex-shrink-0 mt-0.5" style={{ color: isPASGenap ? '#166534' : ACCENT_DARK }} />
                        <p className="text-sm font-medium" style={{ color: isPASGenap ? '#166534' : '#7a3a0a' }}>
                ...
                <div className="overflow-x-auto scrollbar-thin">
                    <div style={{ width: '100%', minWidth: '900px' }}>
                        <div className="grid" style={{ gridTemplateColumns: GRID_COLS, background: BRAND_GRADIENT }}>
                            {['No.', 'Nama Siswa', 'NIS', 'NISN', 'Catatan Wali Kelas', ...(isPASGenap ? ['Naik Tingkat'] : []), 'Aksi'].map((h, i) => (
                        ...
                        {loading ? (
                            Array.from({ length: 5 }).map((_, i) => (
                                <div key={i} className="grid border-b" style={{ gridTemplateColumns: GRID_COLS, borderColor: '#f0f0f0' }}>
                                    {Array.from({ length: isPASGenap ? 7 : 6 }).map((__, j) => (
                                ...
                                >
                                    ...
                                    <div className="px-4 py-4 flex items-center overflow-hidden">
                                        {siswa.catatan_wali_kelas ? (
                                            <p className="text-xs text-gray-700 line-clamp-2 leading-relaxed">{siswa.catatan_wali_kelas}</p>
                                        ) : (
            ...
            {showEdit && editId !== null && (
                <div className={`fixed inset-0 flex items-center justify-center z-50 p-3 sm:p-4 transition-opacity duration-200 ${editClosing ? 'opacity-0' : 'opacity-100'}`} onClick={e => { if (e.target === e.currentTarget) closeEdit(); }}>
                    ...
                    <div className={`relative bg-white rounded-2xl w-full max-w-2xl max-h-[88vh] overflow-y-auto transform transition-all duration-200 ${editClosing ? 'opacity-0 scale-95' : 'opacity-100 scale-100'}`} style={{ boxShadow: '0 20px 50px rgba(0,0,0,0.18)' }}>
                        <div className="sticky top-0 flex items-center justify-between px-4 sm:px-6 py-4 rounded-t-2xl z-10" style={{ background: BRAND_GRADIENT }}>
                            ...
                            <button onClick={closeEdit} className="w-8 h-8 rounded-lg flex items-center justify-center transition-colors hover:bg-white/20" style={{ background: 'rgba(255,255,255,0.15)' }}><X size={16} className="text-white" /></button>
                        </div>
                        <div className="p-4 sm:p-6 space-y-5">
                            <div className="flex items-center gap-4 px-4 sm:px-5 py-4 rounded-xl" style={{ background: '#fff7ed', border: '1px solid #fde0c8' }}>
                                <div className="w-12 h-12 rounded-full flex items-center justify-center flex-shrink-0 text-base font-bold text-white" style={{ background: BRAND_GRADIENT }}>{siswaList.find(s => s.id_siswa === editId)?.nama.charAt(0).toUpperCase() || '?'}</div>
                                <div>
                            ...
                            {isPASGenap ? (
                                ...
                                <div className="flex items-start gap-3 px-4 py-3 rounded-xl" style={{ background: '#eff6ff', border: '1px solid #bfdbfe' }}>
                                    <Info size={18} className="flex-shrink-0 mt-0.5" style={{ color: '#1d4ed8' }} />
                    ...
                    <div className="relative bg-white rounded-2xl w-full max-w-lg dg-scaleIn" style={{ boxShadow: '0 20px 50px rgba(0,0,0,0.18)' }}>
                        <div className="flex items-center justify-between px-4 sm:px-6 py-4 rounded-t-2xl" style={{ background: BRAND_GRADIENT }}>
                            <div className="flex items-center gap-2.5 min-w-0">
                                ...
                                <div className="min-w-0">
                                    ...
                                    <h2 className="text-sm font-bold text-white leading-tight truncate">Import Catatan Wali Kelas</h2>
                                    <p className="text-[10px] text-white/70 mt-0.5">Kelas {kelasNama} - {jenisPenilaian} {semester}</p>
                                </div>
                            </div>
                        ...
                        <div className="p-6">
                            <div className="mb-5 p-4 rounded-xl" style={{ background: '#eff6ff', border: '1px solid #bfdbfe' }}>
                                <p className="text-sm text-blue-900 font-semibold mb-2 flex items-center gap-2"><Info size={16} className="text-blue-600" />Langkah-langkah Import:</p>
                            ...
                            <div className="mb-5 p-3 rounded-xl flex items-start gap-2" style={{ background: '#fff7ed', border: '1px solid #fde0c8' }}>
                                ...
                                <div className="text-xs space-y-1" style={{ color: '#7a3a0a' }}>
                                    ...
                                    {jenisPenilaian === 'PAS' && semester === 'Genap' ? (
                                        <>
                                            <p>- Yang diimport: Catatan dan keputusan naik tingkat</p>
                                            <p className="mt-1"><strong>Tip:</strong> Kedua kolom wajib diisi.</p>
                                        </>
                                    ) : (
                                        <>
                                            <p>- Yang diimport: Hanya catatan wali kelas</p>
                                            <p className="mt-1"><strong>Tip:</strong> Keputusan naik tingkat tidak tersedia di periode ini.</p>
                                        </>
                                    )}
                                </div>
                            </div>
                            ...
                            <div className="flex gap-2.5">
                                ...
                                <ActionButton variant="success" fullWidth disabled={!importFile || importing} onClick={executeImport}>
                                    {importing ? (<><div className="w-4 h-4 rounded-full border-2 border-current/40 border-t-current animate-spin" />Mengimport...</>) : (<><Upload size={16} /> Import Catatan</>)}
                                </ActionButton>
                            </div>
                        </div>
                    </div>
